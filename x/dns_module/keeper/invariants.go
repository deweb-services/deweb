...
// SupplyInvariant checks that the total amount of Domains on collections matches the total amount owned by addresses
	...
	return func(ctx sdk.Context) (string, bool) {
		...
		for denom, supply := range ownersCollectionsSupply {
			if supply != k.GetTotalSupply(ctx, denom) {
				count++
				msg += fmt.Sprintf(
					"total %s Domains supply invariance:\n"+
						"\ttotal %s Domains supply: %d\n"+
						"\tsum of %s Domains by owner: %d\n",
